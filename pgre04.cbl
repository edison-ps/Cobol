      ***************************************************************
      *                                                             *
      *  A B A V / S P - I N F O R M A T I C A    :::  PGRE04       *
      *                                                             * 
      *-------------------------------------------------------------*
      *                                                             *
      ...
      *
       01 cb-prog.
          02 cb-programa               pic x(08) value "PGRE04".
      ...
      *
       01 tela-04.
          ...
          02 line 22 column 42 foreground-color 02 background-color 03 
             highlight value "N".
          02 line 22 column 43 foreground-color 05 background-color 03
      ...
      *
       rot-move-doc.
           move be01-documento to doc-documento.
           move be01-condicao to doc-condicao ab01-condicao.
           move be01-codigo to doc-codigo ab01-codigo.
           perform rot-le-ab01.
           if erro not = 0
           ...
           move be01-portador to doc-portador.
           move be01-operacao to doc-operacao.
           if be01-liquidacao not = 0
           ...
           move be01-valor-pago to doc-valor.
      *
       rot-move-cod.
           move be01-documento to cod-documento.
           move be01-condicao to cod-condicao ab01-condicao.
           move be01-codigo to cod-codigo ab01-codigo.
           perform rot-le-ab01.
           if erro not = 0
           ...
           move be01-portador to cod-portador.
           move be01-operacao to cod-operacao.
           if be01-liquidacao not = 0
              move be01-liquidacao to dias-corr
              move 1 to opcao-data
              perform rot-data
           ...
           move be01-valor-pago to cod-valor.
      *
       rot-move-liq.
           move be01-documento to liq-documento.
           move be01-condicao to liq-condicao ab01-condicao.
           move be01-codigo to liq-codigo ab01-codigo.
           perform rot-le-ab01.
           if erro not = 0
           ...
           move be01-portador to liq-portador.
           move be01-operacao to liq-operacao.
           if be01-liquidacao not = 0
              move be01-liquidacao to dias-corr
              move 1 to opcao-data
              perform rot-data
           ...
           move be01-valor-pago to liq-valor.
      *
       rot-cabec.
       ...
       sec-cns-00.
           ...
           evaluate true
                    ...
                    when sele-ord = 2
                         ...
                         move sele-codigo-i to be01-codigo
                         move sele-condicao to be01-condicao
                         start arqbe01 key is not less be01-chave-1
                    when sele-ord = 3
                         move low-values to be01-chave-2
                         move sele-liq-i to be01-liquidacao
      ...
      *
       lab-cns-01.
           ...
           if be01-codigo < sele-codigo-i or be01-codigo > sele-codigo-f
              go to lab-cns-01
           end-if.
           if be01-liquidacao < sele-liq-i or 
              be01-liquidacao > sele-liq-f
              go to lab-cns-01
           end-if.
           if sele-condicao not = spaces
              if be01-condicao not = sele-condicao
                 go to lab-cns-01
              end-if
            end-if.
           if sele-portador not = 0
              if be01-portador not = sele-portador
                 go to lab-cns-01
              end-if
           end-if.
           if sele-operacao not = 0
              if be01-operacao not = sele-operacao
                 go to lab-cns-01
              end-if
           end-if.
           if sele-contas = 1
              if be01-valor-pago = 0
                 go to lab-cns-01
              end-if
           else
              if sele-contas = 2
                 if be01-valor-pago not = 0
                    go to lab-cns-01
                 end-if
              end-if
           end-if.
           evaluate true
                    when sele-ord = 2
                         if be01-chave-1 not = chave-aux and
                            sub-total not = 0
                    ...
                    when sele-ord = 3
                         if be01-liquidacao not = data-ant and 
                            sub-total not = 0
                            move tracos-c to linha-detalhe
                            perform rot-print
           ...
           evaluate true
                    ...
                    when sele-ord = 2
                         ...
                         if ab01-razao-social-a not = razao-aux
                            ...
                            move be01-chave-1 to chave-aux
                         end-if
                         move det-cod-02 to linha-detalhe
                         perform rot-print
                    when sele-ord = 3
                         perform rot-move-liq
                         if be01-liquidacao = data-ant 
                         ...
                         perform rot-print
                         move be01-liquidacao to data-ant
           end-evaluate.
           if sele-contas not = 2
              add be01-valor to sub-total total
           end-if.
           go to lab-cns-01.
      * 
       lab-cns-fim.
